...
async def create_profile(client):
    resp = await client.post(
        "/api/profile",
        json={"full_name": "Test User", "phone": "+79999999999", "marketing_opt_in": False},
    )
...
async def test_favorites_idempotency_and_delete(async_client, film_ids):
    ...
    resp3 = await client.request("DELETE", FAVORITES_URL, json={"film_id": film_id})
    assert resp3.status_code == 204
    resp4 = await client.request("DELETE", FAVORITES_URL, json={"film_id": film_id})
...
async def test_favorites_pagination(async_client, film_ids):
    ...
    # Get last
    resp3 = await client.get(FAVORITES_URL + "?limit=2&offset=4")
