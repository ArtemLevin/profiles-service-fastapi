...
async def test_rating_validation_valid(client, bearer_token, rating):
    film_id = str(uuid4())
    r = await client.put(f"/api/profile/me/ratings?film_id={film_id}", json={"film_id": film_id, "rating": rating}, headers={"Authorization": bearer_token})
...
async def test_rating_update(client, bearer_token):
    ...
    # Проверяем get
    r = await client.get(f"/api/profile/me/ratings?film_id={film_id}", headers={"Authorization": bearer_token})
