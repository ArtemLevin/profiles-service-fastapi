...
async def ensure_profile(client):
    payload = {
...
async def test_rating_aggregate_cache_and_invalidate(client, bearer_token, redis):
    ...
    data_cached = json.loads(cached)
    assert float(data_cached["avg_rating"]) == 7.5
    assert int(data_cached["ratings_count"]) == 1
